...
class SiteController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					...
					[
						'actions' => ['spv', 'counties', 'localities'],
						'allow' => true,
						'roles' => ['@', '?'],
					],
				],
	...
	public function actionIndex()
	{
        ...
        if ($newsletterModel->load(Yii::$app->request->post()) && $newsletterModel->validate()) {
            if ($newsletterModel->save()) {
                Yii::$app->session->setFlash('success', Yii::t('frontend', 'Thank you for subscribing to our newsletter. From now on you are closer to the news you will receive from us.'));
            } else {
                Yii::$app->session->setFlash('error', Yii::t('common', 'This email have been already submitted to newsletter.'));
            }
		...
		// Map color variables
		$mapColor = '#ffa019';
		$mapHoverColor = '#0e9ce6';
		$mapSelectedColor = '#0e9ce6';

		$romaniaMap = [
			'config' => [
				'mapWidth' => 612,
				'mapHeight' => 434,
				'defaultText' => '<h1>Romania</h1>'
			],
			'paths' => [
				[
					'enable' => true,
				...
				[
					...
					'slug' => Inflector::slug('Bistrița-Năsăud'),
					'textX' => 279,
					'textY' => 95,
					'color' => $mapColor,
				...
				[
					...
					'abbreviation' => 'CS',
					'name' => 'Caraș-Severin',
					'slug' => Inflector::slug('Caraș-Severin'),
					'textX' => 118,
					'textY' => 291,
					'color' => $mapColor,
...
//			'icon' => Yii::getAlias('@web/img/map-marker.png'),
	...
	public function actionSignup()
	{
		...
		if (!empty(Yii::$app->request->get('referral'))) {
			$referral = Subscriber::find()
				->active(true)
				->deleted(false)
				->where([
					'code' => Yii::$app->request->get('referral')
				])
				->one();
			if (!empty($referral)) {
				Yii::$app->session->set('referral', Yii::$app->request->get('referral'));
			} else {
				Yii::$app->session->remove('referral');
			}
		}
		...
		if (($model->load(Yii::$app->request->post()) || !empty($token)) && $model->validate()) {
			if ($model->activate()) {
				...
				if (Yii::$app->getUser()->login($model->getUser(), (int) Yii::$app->settings->get('userLoginDuration'))) {
					...
					if ($subscription->id) {
						return $this->redirect(['/account/payment/subscription', 'id' => Yii::$app->security->maskToken((string) $subscription->id)]);
					} else {
	...
	public function actionSitemap()
	{
        ...
        $announcementsPage = Page::findPageByRoute(['/announcement/default/index']);
        /** @var \common\models\PageTranslation[] $productsPageTranslations */
        $announcementPageTranslations = $announcementsPage->getPageTranslations()
            ->where([
        ...
        $companiesPage = Page::findPageByRoute(['/firm/default/index']);

        $companiesPageTranslations = $companiesPage->getPageTranslations()
            ->where([
                'language_id' => ArrayHelper::getColumn(Language::findAllLanguages(), 'language_id')
            ])
            ->indexBy('language_id')
        ...
        $countyDataProvider = new ActiveDataProvider([
            'query' => County::find()
                ->alias('c')
                ->select([
                    'c.*',
            ...
            'query' => Category::find()
                ...
                ->joinWith([
                    'categoryTranslations ct' => function (ActiveQuery $query) {
                        return $query->andOnCondition([
                            'ct.language_id' => ArrayHelper::getColumn(Language::findAllLanguages(), 'language_id'),
		...
		return Sitemap::widget([
			'items' => [
                ...
                [
                    ...
                    'i18n' => [
                        'relation' => 'announcementTranslations',
                        'route' => function ($item, $model, $modelTranslation) use ($announcementsPage, $announcementPageTranslations) {
                            return [
                                implode('/', [
                                    $announcementPageTranslations[$modelTranslation->language_id]->slug,
                    ...
                    'i18n' => [
                        ...
                        'route' => function ($item, $model, $modelTranslation) use ($announcementsPage, $announcementPageTranslations) {
                            return [
                                implode('/', [
                                    $announcementPageTranslations[$modelTranslation->language_id]->slug,
                                    Inflector::slug(Yii::t($announcementsPage->module, 'category', [], $modelTranslation->language_id)),
                                    $modelTranslation->slug
                                ])
                            ];
                        },
                ...
                [
                    ...
                    'i18n' => [
                        'relation' => 'companyTranslations',
                        'route' => function ($item, $model, $modelTranslation) use ($companiesPage, $companiesPageTranslations) {
                            return [
                                implode('/', [
                                    $companiesPageTranslations[$modelTranslation->language_id]->slug,
                                    $modelTranslation->slug
                                ]),
                            ];
                        },
                ...
                [
                    ...
                    'i18n' => [
                        'route' => function ($item, $model, $modelTranslation) use ($announcementsPage, $announcementPageTranslations, $countyDataProvider) {
                            return [
                                implode('/', [
                                    $announcementPageTranslations[$modelTranslation->language_id]->slug,
                                    Inflector::slug($countyDataProvider->name),
                                    $modelTranslation->slug
                                ])
                            ];
                        },
    ...
    public function actionRss()
    {
        ...
        $dataProvider = new ActiveDataProvider([
            'query' => Announcement::provideRssAnnouncements(),
            'pagination' => [
                'pageSize' => 10,
            ],
        ]);
        ...
        return RssView::widget([
            ...
            'channel' => [
                ...
                'description' => Yii::t('common', 'Announcements'),
            ...
            'items' => [
                'title' => function (Announcement $model, $widget, Feed $feed) {
                    return $model->translation->title;
                },
                'description' => function (Announcement $model, $widget, Feed $feed) {
                    return StringHelper::truncateWords($model->translation->description, 50);
                },
                'link' => function (Announcement $model, $widget, Feed $feed) {
                    return Url::to(['/announcement/default/view', 'slug' => $model->translation->slug], true);
                },
                'guid' => function (Announcement $model, $widget, Feed $feed) {
                    return Url::to(['/announcement/default/view', 'slug' => $model->translation->slug], true);
                },
                'pubDate' => function (Announcement $model, $widget, Feed $feed) {
	...
	{
		$dataset = [];
		if (!($criteria = Yii::$app->request->post('q', null)) && !($criteria = Yii::$app->request->post('l', null)) && !($criteria = Yii::$app->request->post('county', null)) && !($criteria = Yii::$app->request->post('locality', null))) {
			return $this->asJson($dataset);
		}
		if ($criteria = Yii::$app->request->post('q', null)) {
			$categories = \common\models\Category::findByCriteria($criteria);
		...
		if ($criteria = Yii::$app->request->post('l', null)) {
			$counties = \common\models\County::findByCriteria($criteria);
			...
			$localities = \common\models\Locality::findByCriteria($criteria);
	...
	public function actionReferral()
	{
		if (Yii::$app->request->get('code')) {
			return $this->redirect(Url::to(['/site/signup', 'referral' => Yii::$app->request->get('code')], true));
		}
		return $this->redirect(Url::base(true) . '/' . (Yii::$app->settings->get('defaultLanguage') == Yii::$app->language ? '' :  mb_substr(Yii::$app->language, 0, 2)));
	}
