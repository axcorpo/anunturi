	...
	public function saveConversation()
	{
		try {
			...
			if (!$conversation) {
			    ...
			    $conversation->updated_at = $date->format('Y-m-d h:i:s');
            }
	...
	public function saveModel()
	{
		...
		try {
			...
			if ($this->announcement) {
                if (Yii::$app->user->id != $this->announcement->created_by) {
                    $this->recipient_id = $this->announcement->created_by;
                }
            }
			if (Yii::$app->request->get('recipient_id')) {
			    $this->recipient_id = Yii::$app->request->get('recipient_id');
            }
            if (!$this->save()) {
				throw new \Exception();
			}
