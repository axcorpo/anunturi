...
use frontend\modules\account\models\ReservationDeleteForm;
use frontend\modules\account\models\ReservationForm;
...
class PlacedReservationController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'dt-reservations', 'update', 'create', 'view'],
						'roles' => ['@'],
					],
                    [
                        'allow' => true,
...
//            ->orderBy(new Expression("GREATEST(COALESCE(a.renewed_at, 0), COALESCE(a.created_at, 0)) DESC"));
    ...
    {
        $model = $this->findModel($id, ReservationDeleteForm::class);

        if ($model->deleteReservation()) {
            Yii::$app->session->setFlash('success', Yii::t('frontend', 'Reservation deleted successfully.'));
        } else {
            Yii::$app->session->setFlash('error', Yii::t('frontend', 'Cannot delete the selected placed reservation.'));
        }
