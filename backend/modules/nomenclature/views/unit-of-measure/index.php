...
$this->title = Yii::t('common', 'Unit Of Measures');
...
if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == UnitOfMeasure::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => UnitOfMeasure::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restoreUnitOfMeasure') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
			...
			'data' => [
				...
				'dt-table' => '#dt-unit-of-measures',
			],
		],
	],
	[
		'visible' => Yii::$app->user->can('deleteUnitOfMeasure'),
		'tag' => 'button',
		'icon' => 'fa fa-trash',
		'options' => [
	...
	[
		'visible' => Yii::$app->user->can('createUnitOfMeasure'),
		'tag' => 'a',
		'url' => ['create'],
		'icon' => 'fa fa-plus',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-unit-of-measures']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
				data.deleted = ' . json_encode($showTrash ? UnitOfMeasure::YES : null) . ';
			}'),
		],
		'order' => [
			[2, 'asc'],
		...
		'columns' => [
			...
			[
				...
				'filter' => ['select', UnitOfMeasure::getBooleanLabels()],
			],
			[
				'data' => 'created_by',
			...
			[
				...
				'filter' => ['select', ArrayHelper::getColumn(UnitOfMeasure::getStatusLabels(), 'label')],
			],
		],
		'rowCallback' => new JsExpression('function (row, data, index) {
			if (data.defaultValue == "' . UnitOfMeasure::YES . '") {
