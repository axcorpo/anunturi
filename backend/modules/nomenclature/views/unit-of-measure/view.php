...
$this->title = Yii::t('common', 'View {item}', ['item' => Yii::t('common', 'Unit Of Measure')]);
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('viewUnitOfMeasure'),
		'tag' => 'a',
		'url' => ['index'],
		'icon' => 'fa fa-list',
		'options' => [
	...
	[
		'visible' => Yii::$app->user->can('updateUnitOfMeasure'),
		'tag' => 'a',
				...
				<?= DetailView::widget([
					...
					'attributes' => [
						[
							...
							'value' => function (UnitOfMeasure $model) {
								return $model->translation->name ?: '&mdash;';
							},
						],
						[
							'format' => 'html',
							'label' => Yii::t('label', 'Symbol'),
							'value' => function (UnitOfMeasure $model) {
								return $model->translation->symbol ?: '&mdash;';
							},
						],
						[
							'format' => 'html',
							'label' => Yii::t('label', 'e-Invoice Code'),
							'value' => function (UnitOfMeasure $model) {
								return $model->code->translation->name ?: '&mdash;';
							},
						],
						[
							'format' => 'html',
				...
				'clientOptions' => [
					...
					'ajax' => [
						...
						'data' => new JsExpression('function (data) {
							data.model = ' . json_encode(\common\models\UnitOfMeasure::class) . '; 
