			...
			<div class="row">
				...
				<div class="col-sm-4">
					<?= $form->field($model, 'variant')->widget(Select2::class, [
						'data' => Template::getVariantLabels()[Template::TYPE_EMAIL],
						'pluginLoading' => false,
						'pluginOptions' => [
							'allowClear' => false,
							'placeholder' => Yii::t('common', 'Choose'),
						...
						'pluginEvents' => [
							'change' => new JsExpression('function (e) {
								var $target = $(e.target),
									$overlayContext = $(".shortcodes");
								$overlayContext.overlay();
								$.get("' . Url::to(['create']) . '", {variant: $target.val()}).done(function (response) {
									if (response.success === true) {
										var $responseHtml = $(response.data),
											$tabContent = $target.closest("form").find(".tab-content");
		
										$tabContent.children(".tab-pane").each(function (index, tabPane) {
											var $tabPane = $(tabPane);
											$tabPane.find(".shortcodes").html($responseHtml.find("#" + $tabPane.attr("id") + " .shortcodes").html());
										});
									}
								}).always(function () {
									$overlayContext.overlay("remove");
								});
							}'),
						],
					]) ?>
