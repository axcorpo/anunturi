...
$this->title = Yii::t('common', 'View {item}', ['item' => Yii::t('common', 'Document Series')]);
...
$this->params['actions'] = [
	...
	[
		'visible' => Yii::$app->user->can('updateDocumentSeries'),
		'tag' => 'a',
	...
	[
		'visible' => Yii::$app->user->can('deleteDocumentSeries') && !$model->default,
		'tag' => 'a',
				...
				<?= DetailView::widget([
					...
					'attributes' => [
						[
							...
							'value' => function (DocumentSeries $model) {
						...
						[
							'format' => 'html',
							'label' => Yii::t('label', 'First Number'),
							'value' => function (DocumentSeries $model) {
								return $model->first_number ?: '&mdash;';
							},
						],
						[
							'format' => 'html',
				...
				'clientOptions' => [
					...
					'ajax' => [
						...
						'data' => new JsExpression('function (data) {
							data.model = ' . json_encode(DocumentSeries::class) . '; 
