...
class DocumentSeriesSearch extends DataTableAction
{
	/**
	 * @inheritdoc
	 */
	public function init()
	{
		...
		$this->query = DocumentSeries::find()
			->alias('ds')
			->select([
				'ds.id',
				'ds.name',
			...
			->andWhere([
				'ds.deleted' => isset($this->requestParams['deleted']) ? $this->requestParams['deleted'] : DocumentSeries::NO,
			])
            ->orderBy(['ds.type' => SORT_ASC]);
	}
	...
	public function formatData(ActiveQuery $query, $columns)
	{
		return ArrayHelper::toArray($query->all(), [
			DocumentSeries::class => [
				'id',
				'action' => function (DocumentSeries $model) {
					$actions = [];

					if ($this->requestParams['deleted'] == DocumentSeries::YES) {
						if (Yii::$app->user->can('restoreDocumentSeries')) {
						...
						if (Yii::$app->user->can('deleteDocumentSeries')) {
						...
						if (Yii::$app->user->can('viewDocumentSeries')) {
						...
						if (Yii::$app->user->can('updateDocumentSeries')) {
						...
						if (Yii::$app->user->can('deleteDocumentSeries') && $model->default == DocumentSeries::NO) {
							$actions[] = Html::a('<span class="fa fa-trash"></span>', ['delete', 'id' => $model->id], [
								'class' => 'action-delete btn btn-xs btn-danger',
				...
				'name' => function (DocumentSeries $model) {
					return $model->name ?: '&mdash;';
				},
                'type' => function (DocumentSeries $model) {
                    return $model->type ? DocumentSeries::getTypeLabels()[$model->type] : '&mdash;';
                },
				'defaultValue' => function (DocumentSeries $model) {
					return $model->default;
				},
				'default' => function (DocumentSeries $model) {
					return Yii::$app->formatter->asBoolean($model->default);
				},
				'created_by' => function (DocumentSeries $model) {
					return $model->creator ? $model->creator->fullName : '&mdash;';
				},
				'created_at' => function (DocumentSeries $model) {
					return $model->created_at ? Yii::$app->formatter->asDatetime($model->created_at) : '&mdash;';
				},
				'updated_at' => function (DocumentSeries $model) {
					return $model->updated_at ? Yii::$app->formatter->asDatetime($model->updated_at) : '&mdash;';
				},
				'status' => function (DocumentSeries $model) {
					$status = DocumentSeries::getStatusLabels()[$model->status];
		...
		foreach ($columns as $column) {
			...
			switch ($column['data']) {
				case 'created_by':
					...
					break;
				case 'created_at':
					$query->$filterOperator(['LIKE', 'ds.created_at', DateHelper::formatAsDate($value)]);
					break;
				case 'updated_at':
					$query->$filterOperator(['LIKE', 'ds.updated_at', DateHelper::formatAsDate($value)]);
					break;
				default:
					if (array_key_exists($column['data'], $schema)) {
						$query->$filterOperator(['LIKE', 'ds.' . $column['data'], $value]);
					}
					break;
			}
		}
	...
	public function applyOrder(ActiveQuery $query, $columns, $order)
	{
        ...
        foreach ($order as $key => $item) {
			...
			switch ($column['data']) {
				...
				default:
                    if (array_key_exists($column['data'], $schema)) {
                        $query->addOrderBy(['ds.' . $column['data'] => $sort]);
                    }
					break;
			}
		}
