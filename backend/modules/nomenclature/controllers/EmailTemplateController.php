...
use backend\modules\nomenclature\models\EmailTemplateForm;
use backend\modules\nomenclature\models\EmailTemplateSearch;
...
class EmailTemplateController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-email-templates'],
						'roles' => ['viewEmailTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createEmailTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateEmailTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteEmailTemplate'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreEmailTemplate'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-email-templates' => EmailTemplateSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Template::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreEmailTemplate')) {
	...
	public function actionCreate()
	{
		...
		$model->variant = Yii::$app->request->get('variant');
		$result = true;
	...
	{
		$model = $this->findModel($id, EmailTemplateForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Template models.
	...
	/**
	 * Restores Template models that are marked as deleted.
	...
	/**
	 * Finds the Template model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Template|EmailTemplateForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		...
		$query->andWhere(['deleted' => Template::NO]);
