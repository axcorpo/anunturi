...
use backend\modules\nomenclature\models\InvoiceTemplateSearch;
...
class InvoiceTemplateController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-invoice-templates'],
						'roles' => ['viewInvoiceTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createInvoiceTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateInvoiceTemplate'],
					],
					[
						'allow' => true,
						'actions' => ['delete'],
						'roles' => ['deleteInvoiceTemplate'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreInvoiceTemplate'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		return [
			'dt-invoice-templates' => InvoiceTemplateSearch::class,
		];
	}
	...
	public function actionIndex()
	{
		if (Yii::$app->request->get('deleted') == Template::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreInvoiceTemplate')) {
	...
	{
		$model = $this->findModel($id, InvoiceTemplateForm::class);
		$result = true;
	 ...
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Template|InvoiceTemplateForm
	 * @throws NotFoundHttpException if no model was found
	 */
