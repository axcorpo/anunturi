...
use backend\modules\tutorial\models\TutorialForm;
use backend\modules\tutorial\models\TutorialSearch;
...
class TutorialController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-tutorials'],
						'roles' => ['viewTutorial'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createTutorial'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateTutorial'],
					],
					[
						'allow' => true,
						'actions' => ['delete', 'bulk-delete', 'delete-file'],
						'roles' => ['deleteTutorial'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreTutorial'],
					],
				],
			],
		];
	...
	{
		return [
			'dt-tutorials' => TutorialSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Tutorial::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreTutorial')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			TagDependency::invalidate(Yii::$app->cache, 'findAllTutorials');
	...
	{
		$model = $this->findModel($id, TutorialForm::class);
		$result = true;

		Yii::$app->eventLog->beginRecord($model);
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllTutorials']));
			Yii::$app->eventLog->endRecord();
	...
	/**
	 * Deletes existing Tutorial models.
	...
	/**
	 * Restores Tutorial models that are marked as deleted.
	...
	/**
	 * Finds the Tutorial model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Tutorial|TutorialForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Tutorial::class;
		...
		$query->andWhere(['deleted' => Tutorial::NO]);
