...
class RenewalSearch extends DataTableAction
{
	/**
	 * @inheritdoc
	 */
	public function init()
	{
		...
		$this->query = Renewal::find()
			...
			->where([
				'r.deleted' => isset($this->requestParams['deleted']) ? $this->requestParams['deleted'] : Renewal::NO,
			]);
	}
	...
	public function formatData(ActiveQuery $query, $columns)
	{
		return ArrayHelper::toArray($query->all(), [
			Renewal::class => [
				'id',
                'action' => function (Renewal $model) {
                    $actions = [];

                    if ($this->requestParams['deleted'] == Renewal::YES) {
                        if (Yii::$app->user->can('restoreRenewal')) {
                        ...
                        if (Yii::$app->user->can('deleteRenewal')) {
                        ...
                        if (Yii::$app->user->can('viewRenewal')) {
                        ...
                        if (Yii::$app->user->can('updateRenewal')) {
                            $actions[] = Html::a('<span class="fa fa-edit"></span>', ['update', 'id' => $model->id], [
                                ...
                                'data' => [
                                    'toggle' => 'tooltip',
                                    'popup-action' => '',
                                    'popup-done' => ['redrawDataTable' => '#dt-renewals'],
                                ],
                            ]);
                        }
                        if (Yii::$app->user->can('deleteRenewal')) {
                ...
                'announcement' => function (Renewal $model) {
                ...
                'price' => function (Renewal $model) {
                    return $model->price ?: '&mdash;';
                },
                'currency' => function (Renewal $model) {
                    return $model->currency ?: '&mdash;';
                },
                'ip_address' => function (Renewal $model) {
                    return $model->ip_address ?: '&mdash;';
                },
				'created_by' => function (Renewal $model) {
					return $model->creator ? $model->creator->fullName : '&mdash;';
				},
				'created_at' => function (Renewal $model) {
					return $model->created_at ? Yii::$app->formatter->asDatetime($model->created_at) : '&mdash;';
				},
				'updated_at' => function (Renewal $model) {
					return $model->updated_at ? Yii::$app->formatter->asDatetime($model->updated_at) : '&mdash;';
				},
				'status' => function (Renewal $model) {
					$status = Renewal::getStatusLabels()[$model->status];
