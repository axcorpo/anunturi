...
use backend\modules\announcement\models\RenewalForm;
use backend\modules\announcement\models\RenewalSearch;
...
class RenewalController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-renewals'],
						'roles' => ['viewRenewal'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-renewals' => RenewalSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Renewal::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('deleteRenewal')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllRenewals']));
	...
	{
		$model = $this->findModel($id, RenewalForm::class);
	...
	/**
	 * Deletes existing Renewal models.
	...
	/**
	 * Restores Renewal models that are marked as deleted.
	...
	/**
	 * Finds the Renewal model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Renewal|RenewalForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Renewal::class;
		...
		$query->andWhere(['deleted' => Renewal::NO]);
