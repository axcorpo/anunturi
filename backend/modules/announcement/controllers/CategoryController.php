...
use backend\modules\announcement\models\CategoryForm;
use backend\modules\announcement\models\CategorySearch;
...
class CategoryController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-categories'],
						'roles' => ['viewCategory'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-categories' => CategorySearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Category::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('deleteCategory')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			...
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findMainCategories']));
			Category::invalidateCategoryCache($model->id);
	...
	{
		$model = $this->findModel($id, CategoryForm::class);
		...
		try {
			...
			if ($isPermanent) {
				foreach ($deletedModels as $deletedModel) {
					...
					Category::invalidateCategoryCache($deletedModel);
				}
			}
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllCategories']));
			$transaction->commit();
	...
	/**
	 * Restores Category models that are marked as deleted.
	...
	/**
	 * Finds the Category model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Category|CategoryForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Category::class;
		...
		$query->andWhere(['deleted' => Category::NO]);
