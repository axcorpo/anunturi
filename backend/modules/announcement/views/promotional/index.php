...
$this->title = Yii::t('common', 'Promotionals');
$this->params['breadcrumbs'][] = $this->title;

if ($showTrash = Yii::$app->settings->get('enableSoftDelete')) {
	$showTrash = Yii::$app->request->get('deleted') == Promotional::YES;
	$this->params['breadcrumbs'][] = [
		'template' => '<li class="page-links">{link}</li>',
		'label' => implode('', [
			Html::a(Yii::t('common', 'Current'), ['index'], ['class' => $showTrash ? '' : 'active']),
			Html::a(Yii::t('common', 'Trash'), ['index', 'deleted' => Promotional::YES], ['class' => $showTrash ? 'active' : '']),
		]),
	];
}
...
$this->params['actions'] = [
	[
		'visible' => Yii::$app->user->can('restorePromotional') && $showTrash,
		'tag' => 'button',
		'icon' => 'fa fa-undo',
		'options' => [
			...
			'data' => [
				...
				'dt-table' => '#dt-promotionals',
			],
		],
	],
	[
		'visible' => Yii::$app->user->can('deletePromotional'),
		'tag' => 'button',
		'icon' => 'fa fa-trash',
		'options' => [
...
<?= DataTable::widget([
	...
	'clientOptions' => [
		...
		'serverSide' => true,
		'ajax' => [
			'url' => Url::to(['dt-promotionals']),
			'method' => 'POST',
			'data' => new JsExpression('function (data) {
   			data.deleted = ' . json_encode($showTrash ? Promotional::YES : null) . ';
			}'),
		],
		'order' => [
			[4, 'asc'],
		],
		'pageLength' => (int) Yii::$app->settings->get('itemsPerPromotional'),
		'lengthMenu' => [
			[25, 50, 100, -1],
		...
		'columns' => [
			...
			[
				...
				'filter' => ['select', ArrayHelper::getColumn(Promotional::getStatusLabels(), 'label')],
			],
		],
	],
]) ?>
