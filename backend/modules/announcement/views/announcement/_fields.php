...
$category = Yii::$app->request->get('_pjax') ? $category : ($model->getCategories()->where(['leaf' => $model::YES])->one())->id;

?>

<?php Pjax::begin(['id' => 'extra-fields']); ?>
	<?php if ($category): ?>
		<?php foreach(\common\models\Field::findAllByCategoryId($category) as $field): ?>
			<div class="row">
				<div class="col-sm-12">
					<?php switch($field->type): case Field::TYPE_TEXT: ?>
						...
						<?php case Field::TYPE_RADIO: ?>
							<?php if ($field->options): ?>
								<?= $form->field($model, Inflector::slug($field->name . '_' . $field->id, '_'))
								->radioList(ArrayHelper::map($field->options, 'value', 'translation.label'))->label($field->translation->label) ?>
							<?php else: ?>
