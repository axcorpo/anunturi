...
$this->title = Yii::t('common', 'View {item}', ['item' => Yii::t('backend', 'Marketing Recipient')]);
	...
	<?= DetailView::widget([
		...
		'attributes' => [
			[
				...
				'value' => function (MarketingRecipient $model) {
			...
			[
				...
				'value' => function (MarketingRecipient $model) {
					return $model->email ? Html::a($model->phone, 'tel:' . $model->phone) : '&mdash;';
				},
			],
            [
                'format' => 'html',
                ...
                'value' => function (MarketingRecipient $model) {
                    if ($model->marketingGroups) {
                        return implode(', ', array_map(function (MarketingGroup $marketingGroup) {
                            return Html::a($marketingGroup->translation->name, ['/marketing-manager/marketing-group/view', 'id' => $marketingGroup->id], ['class' => 'btn btn-xs btn-default']);
                        }, $model->marketingGroups));
                    }
                    return '&mdash;';
                },
            ],
				...
				'clientOptions' => [
					...
					'ajax' => [
						...
						'data' => new JsExpression('function (data) {
							data.model = ' . json_encode(MarketingRecipient::class) . '; 
