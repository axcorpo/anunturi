...
use backend\modules\website\models\MenuForm;
use backend\modules\website\models\MenuSearch;
...
class MenuController extends MainController
{
	...
	{
		if (Yii::$app->request->get('deleted') == Menu::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreMenu')) {
	...
	{
		$model = $this->findModel($id, MenuForm::class);
		$result = true;
	...
	/**
	 * Restores Menu models that are marked as deleted.
	...
	/**
	 * Finds the Menu model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Menu|MenuForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Menu::class;
		...
		$query->andWhere(['deleted' => Menu::NO]);
