...
use backend\modules\commercial\models\CommercialForm;
use backend\modules\commercial\models\CommercialSearch;
...
class CommercialController extends MainController
{
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-commercials'],
						'roles' => ['viewCommercial'],
					],
					[
						'allow' => true,
						'actions' => ['create'],
						'roles' => ['createCommercial'],
					],
					[
						'allow' => true,
						'actions' => ['update'],
						'roles' => ['updateCommercial'],
					],
					[
						'allow' => true,
						'actions' => ['delete', 'delete-file'],
						'roles' => ['deleteCommercial'],
						'verbs' => ['POST'],
					],
					[
						'allow' => true,
						'actions' => ['restore'],
						'roles' => ['restoreCommercial'],
						'verbs' => ['POST'],
					],
				],
			],
	...
	{
		if (Yii::$app->request->get('deleted') == Commercial::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('deleteCommercial')) {
	...
	/**
	 * Deletes existing Commercial models.
	...
	/**
	 * Restores Commercial models that are marked as deleted.
	...
	/**
	 * Finds the Commercial model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Commercial|CommercialForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		...
		$query->andWhere(['deleted' => Commercial::NO]);
