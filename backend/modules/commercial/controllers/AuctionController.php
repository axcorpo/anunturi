...
use backend\modules\commercial\models\AuctionSearch;
use backend\modules\commercial\models\AuctionForm;
...
class AuctionController extends MainController
{
	/**
	 * @inheritdoc
	 */
		...
		return [
			'access' => [
				...
				'rules' => [
					[
						'allow' => true,
						'actions' => ['index', 'view', 'dt-auctions'],
						'roles' => ['viewAuction'],
					],
					[
						'allow' => true,
	...
	{
		return [
			'dt-auctions' => AuctionSearch::class,
		];
	}
	...
	{
		if (Yii::$app->request->get('deleted') == Auction::YES) {
			if (!Yii::$app->settings->get('enableSoftDelete') || !Yii::$app->user->can('restoreAuction')) {
	...
	public function actionCreate()
	{
		...
		if ($model->load(Yii::$app->request->post()) && ($result = $model->saveModel())) {
			Yii::$app->trigger('invalidate.cache', new \tws\caching\CacheEvent(['key' => 'findAllAuctions']));
	...
	{
		$model = $this->findModel($id, AuctionForm::class);
		$result = true;
	...
	/**
	 * Deletes existing Auction models.
	...
	/**
	 * Restores Auction models that are marked as deleted.
	...
	/**
	 * Finds the Auction model(s) based on its primary key value.
	 * A 404 HTTP exception will be thrown if no record was found.
	 *
	 * @param int|array $id
	 * @param \yii\db\ActiveRecord|null $modelName
	 * @param bool $asActiveQuery
	 * @return \yii\db\ActiveQuery|\yii\db\ActiveRecord|Auction|AuctionForm
	 * @throws NotFoundHttpException if no model was found
	 */
	protected function findModel($id, $modelName = null, $asActiveQuery = false)
	{
		$modelName = class_exists($modelName) ? $modelName : Auction::class;
		...
		$query->andWhere(['deleted' => Auction::NO]);
